...
Default evidence policy: source URL/ID, returned source fields, and observation time. Add screenshots only when explicitly requested, when a field exists only in the UI, or when identity/result ambiguity needs visual evidence.

## Sheet context and schema

For an existing sheet:

1. Read the latest structured state, limited to the named tab and required columns when the protocol permits.
...
Batch API-capable sources before browser work. Use one browser session only for sources or unresolved records that require it. For each entity:
...
4. Store source URL/ID, observation time, and uncertainty. Store a screenshot path only when required by the evidence policy.
...
2. Re-read immediately before writing; keep the live version, resolved row/column IDs, target old values, and headers as write preconditions.
...
7. Retry transient transport failures only at the current read/write stage. Do not repeat completed research.
...
6. If values mismatch, rebuild the plan and retry only mismatched records once.
